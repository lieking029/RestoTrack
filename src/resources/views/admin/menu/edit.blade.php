...
<div class="menu-edit-container">
    <div class="page-header mb-4">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                ...
                <p class="text-muted">Update {{ $menu->name }}</p>
            </div>
    ...
    <form action="{{ route('admin.menu.update', $menu->id) }}" method="POST" enctype="multipart/form-data" id="menuForm">
        @csrf
        @method('PUT')
        ...
        <div class="row">
            ...
            <div class="col-lg-8">
                <div class="card border-0 shadow-sm mb-4">
                    ...
                    <div class="card-body">
                                ...
                                <div class="input-group">
                                    ...
                                    <select class="form-select @error('category') is-invalid @enderror" 
                                        ...
                                        @foreach(\App\Enums\MenuType::toSelectArray() as $value => $label)
                                            <option value="{{ $value }}" {{ old('category', $menu->category->value) == $value ? 'selected' : '' }}>
                                                {{ $label }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="mb-3">
                            <label class="form-label">Current Dish Image</label>
                            <div class="mb-2">
                                <img src="{{ $menu->dish_picture_url }}" alt="{{ $menu->name }}" class="img-thumbnail" style="max-height: 200px;">
                            </div>
                            
                            <label for="dish_picture" class="form-label">Update Image (Optional)</label>
                            <input type="file" 
                ...
                <div class="card border-0 shadow-sm mb-4">
                    ...
                    <div class="card-body">
                        <div class="alert alert-info">
                            <i class="fas fa-info-circle"></i> Update the ingredients and quantities for this dish.
                        </div>

                        <div id="ingredientsContainer">
                            @forelse($menu->products as $index => $product)
                                <div class="ingredient-row mb-3">
                                    <div class="row">
                                        <div class="col-md-8">
                                            <label class="form-label">Ingredient</label>
                                            <select class="form-select ingredient-select" name="ingredients[{{ $index }}][product_id]">
                                                <option value="">Select Ingredient</option>
                                                @foreach($products as $prod)
                                                    <option value="{{ $prod->id }}" 
                                                            data-unit="{{ \App\Enums\UnitOfMeasurement::getLabel($prod->unit_of_measurement->value) }}"
                                                            {{ $product->id == $prod->id ? 'selected' : '' }}>
                                                        {{ $prod->name }} ({{ $prod->remaining_stock }} {{ \App\Enums\UnitOfMeasurement::getLabel($prod->unit_of_measurement->value) }} available)
                                                    </option>
                                                @endforeach
                                            </select>
                                        </div>
            ...
            <div class="col-lg-4">
                    ...
                    <div class="card-body">
                        <div class="form-check form-switch">
                            <input class="form-check-input" 
                                   ...
                                   name="status" 
                                   value="0"
                                   {{ old('status', $menu->status->value) == 0 ? 'checked' : '' }}>
                            <label class="form-check-label" for="status">
                                Available for Order
                            </label>
                        </div>
                        <small class="text-muted">Current: <span class="badge bg-{{ $menu->status_badge_class }}">{{ $menu->status->description }}</span></small>
                    </div>
                </div>
                ...
                <div class="card border-0 shadow-sm mb-4">
                    ...
                    <div class="card-body">
                        <p class="mb-2"><strong>Created:</strong><br>{{ $menu->created_at->format('M d, Y') }}</p>
                        <p class="mb-0"><strong>Last Updated:</strong><br>{{ $menu->updated_at->diffForHumans() }}</p>
                    </div>
                </div>
                ...
                <div class="card border-0 shadow-sm">
                    <div class="card-body">
                        ...
                        <a href="{{ route('admin.menu.show', $menu->id) }}" class="btn btn-outline-info w-100">
...
<script>
    let ingredientIndex = {{ $menu->products->count() }};
